            ...
            <div class="border-gray-200 px-4 py-5 sm:px-6">
                <form class="space-y-6" wire:submit.prevent="publish">
                    <div>
                        ...
                        <x-contentful::input.textarea wire:model.defer="post.title" sharedBorder placeholder="{{ __('Page title…') }}" rows="1" class="border-transparent rounded-md font-bold text-4xl leading-none resize-none" />
                    </div>
                    <div>
                        <label for="slug" class="sr-only">{{ __('Slug') }}</label>
                        <x-contentful::input.text type="text" wire:model.defer="post.slug" sharedBorder placeholder="{{ __('Page slug…') }}" class="border-transparent rounded-md leading-none" />
                    </div>
                    <div>
